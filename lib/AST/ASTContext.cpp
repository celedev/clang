...
std::string ASTContext::getObjCEncodingForBlock(const BlockExpr *Expr,
  ...
  // Encode result type.
  getObjCEncodingForType(BlockTy->getAs<FunctionType>()->getReturnType(), S, 0, EncodeOptionsMask);  
...
std::string ASTContext::getObjCEncodingForFunctionDecl(const FunctionDecl *Decl,
                                                       unsigned EncodeOptionsMask,
                                                       bool WithOffsets) const {
    std::string S;
  // Encode result type.
  getObjCEncodingForType(Decl-> getReturnType(), S, 0, EncodeOptionsMask);
...
std::string ASTContext::getObjCEncodingForMethodDecl(const ObjCMethodDecl *Decl,
  ...
  getObjCEncodingForMethodParameter(Decl->getObjCDeclQualifier(),
                                    Decl->getReturnType(), S, EncodeOptionsMask);
  CharUnits PtrSize = getTypeSizeInChars(VoidPtrTy);
  if (WithOffsets) {
  // Compute size of all parameters.
  // Start with computing size of a pointer in number of bytes.
